...
export default function EditFBGroupPage() {
  const params = useParams();
  const router = useRouter();
  const id = decodeURIComponent(params.id as string);

  const [group, setGroup] = useState<FBGroup | null>(null);
  ...
  useEffect(() => {
    (async () => {
      const groups: FBGroup[] = await fetch("/api/admin/fb-groups").then((r) => r.json());
      const found = groups.find((g) => g.id === id);
      if (!found) { setNotFound(true); return; }
      setGroup(found);
      setName(found.name);
      setUrl(found.url);
      setSnippet(found.snippet);
  ...
  async function save() {
    ...
    const updated = groups.map((g) =>
      g.id === id ? { ...g, name, url, snippet, members, weeklyPostFrequency, intent, trades, locations } : g
    );
    ...
    setSaved(true);
    setTimeout(() => router.push("/admin/fb-groups"), 600);
  }
  ...
  if (notFound) {
    return (
      <div className="p-8 max-w-2xl">
        ...
        <Link href="/admin/fb-groups" className="text-sm text-blue-600 hover:underline mt-2 inline-block">
          ← Back to FB Groups
        </Link>
      </div>
    );
  ...
  return (
    <div className="p-8 max-w-2xl">
      ...
      <div className="mb-6">
        ...
        <div className="flex items-center gap-2 mb-1">
          <Link href="/admin/fb-groups" className="text-sm text-gray-400 hover:text-gray-600 transition-colors">
            FB Groups
          </Link>
          <span className="text-gray-300">/</span>
          <span className="text-sm text-gray-600 font-medium">Edit</span>
        </div>
        <h1 className="text-2xl font-semibold text-gray-900">Edit FB Group</h1>
        <p className="mt-1 text-xs text-gray-400 font-mono">ID: {id}</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
        ...
        {/* Members + Weekly Post Frequency — side by side */}
        <div className="flex gap-4">
          <div className="flex-1">
            ...
            <input
              ...
              min={0}
              value={members}
              onChange={(e) => setMembers(Math.max(0, parseInt(e.target.value) || 0))}
